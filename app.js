...
})) // dont understand how this works... why () outside {}?
...
inventorNamedAda = inventors.find((inventor) => {
 return inventor.first === 'Ada'; // why we need to put return? isnt arrow function has built-in return?
})
...
firstLast = people.map((person)=> {
    const reversedPerson = person.split(", ").reverse().join(", ");
    return reversedPerson; // this one return the final result but the rest doesnt.
});
...
let isAdultPresent = null; //why let this be null?

// Complete the exercise in the space below:

isAdultPresent = devs.some((person)=> {
    return (2025 - person.year) > 18;
})
...
commentById = comments.find((comment)=>{
    if(comment.id === 823423){
        return comment.text //isnt this should only return 'Super good'?
    };
})
...
}, 0); //why without ', 0' doesnt work?

// why this format doesnt work?
totalYearsLived = inventors.reduce((total, inventor)=> total + inventor.passed - inventor.year);
...
acc[method] = acc[method] + 1;
    ...
    return acc;
},{}); // i dont understand this at all. i just follow the tutorial structure.
// is it acc is an array? not a box that contains value. so acc["car"] will create "car":?
// acc[method] = acc[method] + 1, left hand is array then right hand side is value?
